...
export default function Login() {
  ...
  const handle = async (e: React.FormEvent) => {
    ...
    if (
      email === 'administracion@casaalaniz.es' &&
      pass === 'Redbull123.'
    ) {
      // Guardar autenticación con timestamp
      localStorage.setItem('alanizAuth', 'ok');
      localStorage.setItem('alanizAuthTimestamp', Date.now().toString());
      
      // Mostrar éxito brevemente antes de navegar
      setError('success');
      setTimeout(() => {
        navigate('/miembros');
      }, 1000);
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="card-elegant animate-fade-in-up shadow-2xl">
          ...
          <form onSubmit={handle} className="form-elegant">
            ...
            {/* Campo de email */}
            <div>
              <label htmlFor="email" className="block text-sm font-medium text-alanizGold-600 mb-2">
                Correo Electrónico
              </label>
              <div className="relative">
                ...
                <input
                  id="email"
                  type="email"
                  placeholder="administracion@casaalaniz.es"
                  value={email}
            ...
            >
              {isLoading ? (
                <div className="flex items-center justify-center space-x-2">
                  <div className="w-4 h-4 border-2 border-alanizGreen-950 border-t-transparent 
          ...
          <div className="mt-8 pt-6 border-t border-alanizGold-600/20">
            <div className="bg-alanizGreen-900/30 rounded-lg p-4">
              ...
              <p className="text-xs text-parchment-400 leading-relaxed mb-3">
                El acceso está restringido a miembros oficiales de la Casa Alaniz 
